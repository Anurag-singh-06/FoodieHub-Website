  ...
  if (data.length === 0) {
    return (
      <div >
        ...
        <div style={{ color: 'white' }} className='py-3 w-100 text-center fs-3'>The Cart is Empty!</div>
        <div className='d-flex justify-content-center py-3'>
        <Link to="/"><button className=' py-1 rounded '>Continue Shopping</button></Link>
        </div>
      </div>
    )
  }
  ...
  return (
    <div>
      ...
      <div className='container m-auto mt-5 table-responsive  table-responsive-sm table-responsive-md' >
        ...
        <div>
          <Link to="/paymentpage"><button className='btn bg-success mt-5 ' onClick={handleCheckOut} > Check Out </button></Link>
        </div>
      </div>
